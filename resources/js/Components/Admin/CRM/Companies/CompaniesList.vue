...
<div class="min-h-full">
  ...
  <header class="bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
      <div class="flex-1 min-w-0">
        <nav class="flex" aria-label="Breadcrumb">
          <ol role="list" class="flex items-center space-x-4">
            <li>
              <div>
                <a :href="route('admin.crm.companies.index')" class="text-sm font-medium text-gray-500 hover:text-gray-700">Companies</a>
              </div>
            </li>
          </ol>
        </nav>
      ...
      <div>
  <label for="email" class="block text-sm font-medium text-gray-700">Search companies</label>
  <div class="mt-1 flex rounded-md shadow-sm">
    <div class="relative flex items-stretch flex-grow focus-within:z-10">
      ...
      <input type="company_name" name="company_name" id="company_name" class="focus:ring-blue-500 focus:border-blue-500 block w-full rounded-none rounded-l-md pl-10 sm:text-sm border-gray-300" placeholder="Company Name">
    </div>
            ...
            <div class="flex items-center py-5 px-4 sm:py-6 sm:px-0">
              <div class="min-w-0 flex-1 flex items-center">
                <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                  <div>
                    <p class="text-sm font-medium text-purple-600 "> <a :href="route('admin.crm.companies.show', company.slug)">{{ company.company_name}}</a> - {{ company.phone }}</p>
                    <p class="mt-2 flex items-center text-sm text-gray-500">
                      ...
                      <span> <a :href="`mailto:${company.email_address}`" >{{ company.email_address }}</a> </span>
                    </p>
                  </div>
                  <div class="hidden md:block">
                    <div>
                      <p class="text-sm text-gray-900">
                        Last Contacted on: 
                        <time datetime="2020-07-01T15:34:56">{{ company.last_contact }}</time>
                      </p>
                      <p v-if="company.is_customer" class="mt-2 flex items-center text-sm text-gray-500">
...
<p v-else class="mt-2 flex items-center text-sm text-gray-500">
                        <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
